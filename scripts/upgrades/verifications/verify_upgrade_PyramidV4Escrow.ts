...
async function main() {
  ...
  // Get the deployed PyramidV3Escrow contract
  ...
  const pyramidV3Before = await ethers.getContractAt(
    'PyramidV3Escrow',
    pyramidAddress,
  );
  console.log('\nChecking state before upgrade:');

  const treasuryBefore = await pyramidV3Before.s_treasury();
  const arkadaRewarderBefore = await pyramidV3Before.s_arkadaRewarder();
  const isMintingActiveBefore = await pyramidV3Before.s_isMintingActive();
  const versionBefore = await pyramidV3Before.pyramidVersion();
  ...
  const PyramidV4Escrow = await ethers.getContractFactory(
    'PyramidV4Escrow',
    ownerSigner,
  );
  ...
  const pyramidV4After = await ethers.getContractAt(
    'PyramidV4Escrow',
    pyramidAddress,
  );
  ...
  const versionAfter = await pyramidV4After.pyramidVersion();
  console.log('Contract version:', versionAfter);
  if (versionAfter !== '4') {
    throw new Error(
      'Invalid contract version. Expected 4, got ' + versionAfter,
    );
  }

  // 2. Verify treasury
  const treasuryAfter = await pyramidV4After.s_treasury();
  ...
  const arkadaRewarderAfter = await pyramidV4After.s_arkadaRewarder();
  ...
  const isMintingActiveAfter = await pyramidV4After.s_isMintingActive();
  ...
  const DEFAULT_ADMIN_ROLE = await pyramidV4After.DEFAULT_ADMIN_ROLE();
  const hasAdminRole = await pyramidV4After.hasRole(
    DEFAULT_ADMIN_ROLE,
    adminAddress,
  );
  ...
  const filter = pyramidV4After.filters.PyramidClaim();
  const events = await pyramidV4After.queryFilter(filter, -10000, 'latest');

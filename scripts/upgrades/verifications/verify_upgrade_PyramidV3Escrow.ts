...
async function main() {
  ...
  const PyramidV3Escrow = await ethers.getContractFactory(
    'PyramidV2Escrow',
    ownerSigner,
  );
  ...
  if (versionAfter !== '3') {
    throw new Error(
      'Invalid contract version. Expected 3, got ' + versionAfter,
    );
  }
  ...
  // 5. Verify contract is upgradeable

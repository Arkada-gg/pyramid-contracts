...
export const checkTest = async (
  ...
  if (opt?.revertMessage) {
    await expect(dailyCheckContract.connect(sender).check()).revertedWith(
      opt?.revertMessage,
    );
    return;
  }

  await expect(dailyCheckContract.connect(sender).check()).to.emit(
    dailyCheckContract,
    dailyCheckContract.interface.events['DailyCheck(address,uint256,uint256)']
      .name,
  ).to.not.reverted;
